...
#define ARA_IPC_DECODE_OPTIONAL_ADDENDUM_STRUCT_PTR(member)                                     \
        ARA_IPC_DECODE_OPTIONAL_STRUCT_PTR(member)                                              \
        if (subDecoderRef_##member != nullptr) {                                                \
            ARA_IPC_UPDATE_STRUCT_SIZE_FOR_OPTIONAL (member);                                   \
        }
...
ARA_IPC_BEGIN_DECODE_SIZED (ARAAudioSourceProperties)
    ...
    if (result.structSize > offsetof (ARAAudioSourceProperties, channelArrangementDataType))
    {
        ...
        else
        {
            ...
            static union
            {
                ARAInt64 speakerArrangement;
                ARAInt32 stemFormat;
                ARAByte audioChannelLayoutBytes[2016UL];    // some arbitrary limit that can be conveniently allocated
            } cache;
            auto resultSize_channelArrangement { sizeof (cache.audioChannelLayoutBytes) };
            BytesDecoder tmp_channelArrangement { cache.audioChannelLayoutBytes, resultSize_channelArrangement };
            if (_readAndDecode (tmp_channelArrangement, decoder, offsetof (ARAAudioSourceProperties, channelArrangement)) &&
                (resultSize_channelArrangement < sizeof (cache.audioChannelLayoutBytes)))
            {
                result.channelArrangement = &cache.audioChannelLayoutBytes;
            }
            else
            {
